// From "Programming Rust" by Blandy, Orendorff, and Tindall
// Some parts modified by Tristan Andrus ("steelswords")
...
fn main() {
    ...
    if args.len() != 5 {
        ...
        eprintln!("Example: {} mandelbrot.png 1000x750 -1.20,0.35, -1,0.20", args[0]);
        std::process::exit(1);
    }
...
/// Try to determine if `c` is in the Mandlebrot set, using at most `limit` iterations
/// to decide
///
...
/// seems to be a member (or, more precisely, if we reached the iteration limit without being able
/// to prove that `c` is not a member), return `None`.
...
fn parse_complex<T: FromStr>(input: &str) -> Option<Complex<T>> {
...
/// The `upper_left` and `lower right` parameters are points on the complex plane designating the
...
fn pixel_to_point(bounds: (usize, usize),
    ...
    Complex { 
        ...
        // but the imaginary complonent increases as we go up.
    }
}
...
///
/// The `bounds` argument gies the width and height of the buffer `pixels`,
